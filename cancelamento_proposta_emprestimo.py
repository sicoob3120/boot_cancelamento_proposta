...
class CancelCred:
    def __init__(self):
        ...
        self.img_emprestimo_app = os.path.join(self.caminho_imagens, 'emprestimo.png')
    ...
    def escolher_fase(self):
        ...
        for fase in fases:
            ...
            while True:
                ...
                screen_text_proposta = self.extract_text_from_image(screen_path, config="-l eng+equ+por --psm 6 -c tessedit_char_whitelist= 0123456789.,-abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ")
    ...
    def extrair_textos_especificos(self, text):
        ...
        cpf_cnpj  = valor_contrato =  pa = numero_proposta = None

        # Extrair CPF/CNPJ
        cpf_cnpj_match = re.search(r'\d{3}\.\d{3}\.\d{3}-\d{2}|\d{2}\.\d{3}\.\d{3}/\d{4}-\d{2}', text)
        if cpf_cnpj_match:
        ...
        logging.debug(f'CPF:{cpf_cnpj}; Valor:{valor_contrato}; PA:{pa}; Contrato:{numero_proposta}')

        return cpf_cnpj, valor_contrato,  pa, numero_proposta
